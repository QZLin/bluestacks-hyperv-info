...
try:
    ...
    HcsEnumerateComputeSystems.argtypes = (
        ctypes.c_wchar_p, ctypes.POINTER(ctypes.c_wchar_p), ctypes.POINTER(ctypes.c_wchar_p))
    ...
    HcsGetComputeSystemProperties.argtypes = (
        HCS_SYSTEM, ctypes.c_void_p, ctypes.POINTER(ctypes.c_wchar_p), ctypes.POINTER(ctypes.c_wchar_p))
    ...
    HcnQueryNetworkProperties.argtypes = (
        HCN_NETWORK, ctypes.c_void_p, ctypes.POINTER(ctypes.c_wchar_p), ctypes.POINTER(ctypes.c_wchar_p))
    ...
    HcnEnumerateEndpoints.argtypes = (
        ctypes.c_wchar_p, ctypes.POINTER(ctypes.c_wchar_p), ctypes.POINTER(ctypes.c_wchar_p))
    ...
    HcnQueryEndpointProperties.argtypes = (
        HCN_ENDPOINT, ctypes.c_void_p, ctypes.POINTER(ctypes.c_wchar_p), ctypes.POINTER(ctypes.c_wchar_p))
                ...
                if policies := obj.get("Policies", None):
                    for policy in policies:
                        if policy.get("InternalPort", None) == 5555:
                            ...
                            # result = True
        return ports if len(ports) != 0 else None
else:
    def run(connector, params):
